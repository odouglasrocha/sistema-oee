...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // Login automático com credenciais baseadas no ambiente
  const autoLogin = async () => {
    try {
      ...
      console.log(`🔄 Tentando login automático (${environment}) com:`, credentials.email);
      ...
      if (!response.ok) {
        ...
        console.error(`❌ Resposta de erro do login:`, response.status, errorText);
        throw new Error(`Falha no login automático: ${response.status}`);
      }
      ...
      if (data.tokens && data.tokens.accessToken) {
        ...
        console.log(`✅ Login automático realizado com sucesso (${environment})`);
        console.log(`✅ Usuário logado:`, data.user.name, data.user.email);
      } else {
      ...
      setError(`Erro no login automático (${environment}): ${error.message}`);
      setUser(null);
    }
  };
